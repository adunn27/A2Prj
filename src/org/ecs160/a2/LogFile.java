...
public class LogFile {
    ...
    private void executeLogLine(String[] split) {
        ...
        switch (split[INT]) {
            case "add":
                ...
                TaskId = Integer.parseInt(split[5]);
                task.setId(TaskId);
                List<String> tags_a= new ArrayList<>();
                for(String tag : split[6].split(TAG_DELIMITER)){
                    tags_a.add(tag);
                }
            ...
            case "edit":
                task = retrieveTask.getTaskById(Integer.parseInt(split[5]));
            ...
            case "stop":
                ...
                task = retrieveTask.getTaskByName(taskName);
                task.getAllTimeSpans().get(task.getAllTimeSpans().size() - INT).setEndTime(taskTime_e);
                task.setInActive();
            ...
            case "edit_time":
                ...
                break;
            default:
                System.out.println("Invalid command: " + split[INT] );
        }
    }
    ...
    private String getTagsAsString(Task task) {
        ...
        for(String tag : task.getTags()){
            tags += tag.replace(TAG_DELIMITER, ESCAPE_CHAR + TAG_DELIMITER);
            tags += TAG_DELIMITER;
        }
        return tags;
    }
